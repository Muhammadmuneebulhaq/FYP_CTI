...
Example usage:
  ...
  python LLM_Validation.py -j merged_final/APT28_merged.json -t Data/APT28.txt --output APT28_stix.json
    ...
    print(f"\nDocument: {merged_json.get('document_name', 'Unknown')}")
    print(f"Entities: {merged_json.get('entities', {}).get('summary', {}).get('total_entities', 0)}")
    print(f"Relations: {merged_json.get('relationships', {}).get('summary', {}).get('total_relations', 0)}")
    ...
    # System instruction as initial history message (role='model')
    system_instruction = """You are a STIX 2.1 validation expert for Cyber Threat Intelligence. Analyze inputs strictly against the provided document text, without inferring or adding external information. Ensure all outputs are STIX-compliant: use valid SDOs (e.g., threat-actor, malware), SCOs (e.g., ipv4-addr), and SROs (e.g., uses, targets). Required attributes include type, id (UUID format), created, modified. Output only valid JSON; if issues exist, include an 'issues' array explaining them. Do not hallucinate-reference only explicit text matches.
STIX Schema Key Excerpts: SDO types include attack-pattern, campaign, threat-actor; SCO types include ipv4-addr, domain-name; Relationship types include uses, targets, related-to. Full details at https://docs.oasis-open.org/cti/stix/v2.1/stix-v2.1.html."""
