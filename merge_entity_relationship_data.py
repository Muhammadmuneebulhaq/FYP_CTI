...
def merge_document_data(merged_file_path, relationship_data, relationship_filename):
    ...
    if not entity_data:
        print(f"  [FAIL] Failed to load entity data")
        return None
...
def process_all_documents(results_folder, relationship_folder, output_folder):
    ...
    if not merged_files:
        print(f"\n[FAIL] No files found in {merged_path}")
        return
    ...
    for merged_file in merged_files:
        ...
        if relationship_file.exists():
            ...
            print(f"\n[WARN] Relationship file not found for {base_name}. Proceeding with entities only.")
        ...
        try:
            ...
            print(f"  [FAIL] Error processing {base_name}: {str(e)}")
            all_docs_summary['failed'] += 1
            all_docs_summary['documents'].append({
                'name': base_name,
                'status': 'failed',
...
def create_consolidated_output(output_folder):
    ...
    print(f"  [OK] Created consolidated file: {consolidated_file.name}")
...
if __name__ == "__main__":
    ...
    print(f"[OK] Individual merged files: {OUTPUT_FOLDER}/<document>_merged.json")
    print(f"[OK] Consolidated file: {OUTPUT_FOLDER}/all_documents_consolidated.json")
