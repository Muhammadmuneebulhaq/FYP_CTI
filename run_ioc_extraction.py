...
def run_ioc_extraction_on_dataset():
    ...
    for i, file_path in enumerate(text_files, 1):
        ...
        try:
            # Skip if file doesn't exist (may have been backed up by streamlit)
            if not file_path.exists():
                print(f"  [SKIP] File not found (may be backed up)")
                skipped_files.append(report_name)
                continue
            ...
            print(f"  [OK] Found {result['ioc_count']} IOCs")

        except Exception as e:
            print(f"  [FAIL] Error processing {report_name}: {e}")
            error_files.append(report_name)
    ...
    print("\n" + "=" * 80)
    print(f"[OK] IOC extraction complete!")
    print(f"  - Processed: {len(all_results)} reports")
    print(f"  - Skipped: {len(skipped_files)} reports (may be backed up)")
    if error_files:
        print(f"  - Errors: {len(error_files)} reports")
