"""
Document Processing Script for TIKG Model
Processes cyber threat documents, extracts entities and relationships using TIKG.
"""
...
from run_tikg import load_tikg_model, predict_tikg
import torch
...
def process_document(file_path, model, tokenizer, relation2id, id2relation, device='cpu', debug=False):
    """
    Process a single document: split into sentences and extract entities/relations using TIKG.
    
    Args:
        file_path: Path to the document file
        model: Loaded TIKG model
        tokenizer: Tokenizer
    ...
    for idx, sentence in enumerate(sentences):
        ...
        try:
            ...
            entities, relations = predict_tikg(
                sentence, model, tokenizer, relation2id, id2relation,
...
def process_all_documents(data_folder, output_folder, model, tokenizer, relation2id, id2relation, device='cpu', debug=False):
    """
    Process all .txt files in the data folder using TIKG model.
    ...
    print(f"\n{'='*80}")
    print(f"Found {len(txt_files)} documents to process using TIKG model")
    print(f"{'='*80}")
    ...
    for idx, file_path in enumerate(txt_files):
        try:
            ...
            doc_results = process_document(
                str(file_path), model, tokenizer, relation2id, id2relation,
                device=device, debug=debug
            )
            
            # Save individual document results
            output_filename = f"{Path(file_path).stem}_tikg_results.json"
    ...
    summary_path = os.path.join(output_folder, "_tikg_all_documents_summary.json")
    with open(summary_path, 'w', encoding='utf-8') as f:
        json.dump(all_documents_summary, f, indent=2, ensure_ascii=False)
    
    print(f"\n{'='*80}")
    print("TIKG PROCESSING COMPLETE!")
    print(f"{'='*80}")
    ...
    print(f"Summary file: _tikg_all_documents_summary.json")
    print(f"{'='*80}\n")

def create_relations_csv(output_folder, csv_filename="tikg_extracted_relations.csv"):
    """
    ...
    for json_file in output_path.glob("*_tikg_results.json"):
    ...
    if all_relations:
        ...
        print(f"[OK] Created CSV file with {len(all_relations)} relations: {csv_filename}")
    else:
        print("[FAIL] No relations found to export to CSV")

# --- MAIN EXECUTION ---
if __name__ == "__main__":
    print("\n" + "="*80)
    print("TIKG Model - Batch Document Processing")
    print("="*80 + "\n")
    
    # Configuration
    TIKG_MODEL_FOLDER = "./TIKG_model"
    DATA_FOLDER = "./Data"
    OUTPUT_FOLDER = "./relationship"
    DEBUG_MODE = False
    ...
    try:
        # Load model
        print("Loading TIKG model...")
        model, tokenizer, relation2id, id2relation = load_tikg_model(
            TIKG_MODEL_FOLDER, device=device
        )
        print("[OK] Model loaded successfully!\n")
        
        # Process all documents
        process_all_documents(
            DATA_FOLDER, OUTPUT_FOLDER, model, tokenizer, relation2id, id2relation,
            device=device, debug=DEBUG_MODE
        )
        ...
        print(f"[OK] Check the '{OUTPUT_FOLDER}' folder for results")
